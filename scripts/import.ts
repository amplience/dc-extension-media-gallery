...
const recursiveTemplateSearch = async (
	...
	targetDir: string,
	dir: string,
	fileFunc: (path: string) => Promise<void>
) => {
...
export const importArgs = (yargs: Argv) => {
	return yargs
		...
		.option('extensionClientId', {
			describe: 'extensionClientId',
			default: null,
			type: 'string'
		})
		.option('extensionClientSecret', {
			describe: 'extensionClientSecret',
			default: null,
			type: 'string'
		})
		.option('extensionVSE', {
			describe: 'extensionVSE',
			default: null,
			type: 'string'
		})
}
...
export const importHandler = async (context: Arguments<Context>): Promise<any> => {
	...
	try {
		...
		execSync(`npx dc-cli --version`, { stdio: 'inherit' })
		...
		execSync(`npx dc-cli content-type-schema import ${context.tempDir}/schema`, {
			stdio: 'inherit'
		})
